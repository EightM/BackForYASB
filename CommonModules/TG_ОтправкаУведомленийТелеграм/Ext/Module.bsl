...
Процедура ОтправитьУведомленияПоНаправленнымОшибкам(ДатаПоследнейРассылки, МассивПроектов, ПараметрыРассылки) Экспорт
	...
	|	И ПерсональныеНастройкиУведомленийПользователей.УведомлятьВTelegramОНаправленииОшибки
	...
	Пока Выборка.Следующий() Цикл
		...
		РеквизитыОшибки.Вставить("Проект", Выборка.Проект);
...
Процедура ОтправитьСообщения(РеквизитыЗадачи)
	...
	Адрес = "/messages/task";
	ОтправитьЗапрос(ДанныеОсновногоБота, Данные, Адрес);
...
Процедура ОтправитьОшибки(РеквизитыОшибки)
	...
	Данные = Новый Структура;
	Если НастройкиПользователя.Свойство("Наименование") <> Ложь Тогда
		Данные.Вставить("name", Строка(РеквизитыОшибки.Наименование));
	КонецЕсли;
	
	Если НастройкиПользователя.Свойство("Статус") <> Ложь Тогда
		Данные.Вставить("status", Строка(РеквизитыОшибки.Статус));
	КонецЕсли;
	
	Если НастройкиПользователя.Свойство("Срочностьисправления") <> Ложь Тогда
		Данные.Вставить("priority", Строка(РеквизитыОшибки.СрочностьИсправления));
	КонецЕсли;

	Если НастройкиПользователя.Свойство("КомуНаправлена") <> Ложь Тогда
		Данные.Вставить("executor", Строка(РеквизитыОшибки.КомуНаправлена));
	КонецЕсли;
	
	Если НастройкиПользователя.Свойство("Комментарий") <> Ложь Тогда
		Данные.Вставить("comment", Строка(РеквизитыОшибки.Комментарий));
	КонецЕсли;

	Если НастройкиПользователя.Свойство("Код") <> Ложь Тогда
		Данные.Вставить("code", Строка(РеквизитыОшибки.Код));
	КонецЕсли;

	Если НастройкиПользователя.Свойство("Порядоквоспроизведения") <> Ложь Тогда
		Данные.Вставить("description", Строка(РеквизитыОшибки.ПорядокВоспроизведения));
	КонецЕсли;
	
	Если НастройкиПользователя.Свойство("Проект") <> Ложь Тогда
		Данные.Вставить("project", Строка(РеквизитыОшибки.Проект));
	КонецЕсли;
	
	Данные.Вставить("recipient_id", РеквизитыОшибки.ТелеграмId);
	Адрес = "/messages/error";
	ОтправитьЗапрос(ДанныеОсновногоБота, Данные, Адрес);
	
КонецПроцедуры

Процедура ОтправитьЗапрос(ДанныеОсновногоБота, Данные, Адрес)
...
Функция ПолучитьНастройкиПоОшибкам(Исполнитель)
	...
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		РеквизитОбъекта = СтрЗаменить(Строка(Выборка.РеквизитОбъекта), " ", "");
		НастройкиСостава.Вставить(РеквизитОбъекта, Выборка.ВключатьВСостав);	
	КонецЦикла; 
